...
BLOCK_MARKER_START="# >>> Zscaler combined CA (managed by PyZTrust.sh) >>>"
BLOCK_MARKER_END="# <<< Zscaler combined CA (managed by PyZTrust.sh) <<<"
...
" 2>"$REALPATH_ERR") || {
    ...
    echo ""
    echo "   Combined CA bundle ready: $COMBINED_CERT"
    echo ""
...
#
# Any existing managed block (delimited by BLOCK_MARKER_START/END) is
# removed before the current export lines are appended directly to the
# file. Each file is backed up before modification, allowing rollback if
# any update fails.
# ---------------------------------------------------------------------------
if [[ -n "$selected_python_env" ]]; then
    ...
    for RC_FILE in "${RC_FILES[@]}"; do
        ...
        set +e
        awk -v start="$BLOCK_MARKER_START" -v end="$BLOCK_MARKER_END" '
            $0 == start { skip=1; next }
            $0 == end   { skip=0; next }
            skip != 1   { print }
        ' "$RC_FILE" > "$RC_TMP"
        ...
        set -e
        if (( awk_exit != 0 )); then
            echo "   ✘ Failed reading $RC_FILE (awk exit $awk_exit) — restoring backup."
        ...
        if grep -qF "$BLOCK_MARKER_START" "$RC_TMP" 2>/dev/null; then
            echo "   ✘ Old managed block was not removed from $RC_FILE."
            prune_tmp_file "$RC_TMP"
            rc_update_failed="true"
            break
        fi
        ...
        if ! mv "$RC_TMP" "$RC_FILE"; then
            ...
            echo "   Export lines written to $RC_FILE (backup at $RC_BACKUP)"
...
if [[ -n "$selected_python_env" ]]; then
    ...
    if [[ ${#RC_FILES[@]} -eq 0 ]]; then
        ...
        echo "  Add these lines to whichever shell startup file you use:"
        echo "    export SSL_CERT_FILE=\"$COMBINED_CERT\""
        echo "    export REQUESTS_CA_BUNDLE=\"$COMBINED_CERT\""
        echo "    export PIP_CERT=\"$COMBINED_CERT\""
    fi
    ...
    for RC_FILE in "${RC_FILES[@]}"; do
        echo "    perl -i -ne 'print unless /^# >>> Zscaler combined CA.*>>>\$/ .. /^# <<< Zscaler combined CA.*<<<\$/' \"$RC_FILE\""
    done
    echo ""
    exit 0
else
